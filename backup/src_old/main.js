...
// Warm atmospheric fog like reference (very subtle)
// scene.fog is set above with sky
...
// Use configured shadow camera size
const shadowSize = config.lighting.sun.shadowCameraSize || 400;
...
// Warm atmospheric sky like reference
scene.background = new THREE.Color(0x9ec9e8); // Soft blue sky

// Warm exponential fog for atmospheric depth
scene.fog = new THREE.FogExp2(0xb8d5e8, 0.00015); // Very subtle warm fog
...
function createAllVegetation() {
  ...
  // Create massive cliffs FIRST (most impactful)
  createMassiveCliffs();
  // Using realistic trees with tall trunks
  createBetterProceduralTrees();
...
// Better realistic trees with improved geometry
function createBetterProceduralTrees() {
  const dtc = config.deciduousTrees;
  const positions = generatePositions(dtc.count, dtc.spreadFactor, dtc.minRiverDistance);

  console.log(`Creating ${positions.length} optimized realistic trees...`);
  ...
  positions.forEach((pos, i) => {
    ...
    const trunkMat = new THREE.MeshStandardMaterial({
      color: 0x4a3020,
      roughness: 0.95,
      flatShading: true // More angular/realistic bark
    });
    ...
    // Canopy - use cones and spheres for more organic shape
    const canopyY = trunkHeight * 0.7;
    const foliageColors = [0x3a7035, 0x2a6028, 0x4a8040, 0x358030];

    // Main canopy - larger cone shape for tree-like silhouette
    const mainConeGeom = new THREE.ConeGeometry(3 * scale, 5 * scale, 8);
    const mainConeMat = new THREE.MeshStandardMaterial({
      color: foliageColors[i % foliageColors.length],
      roughness: 0.9,
      flatShading: false
    });
    const mainCone = new THREE.Mesh(mainConeGeom, mainConeMat);
    mainCone.position.y = canopyY + 2 * scale;
    mainCone.castShadow = true;
    mainCone.receiveShadow = true;
    treeGroup.add(mainCone);

    // Add 2-3 smaller spheres for fuller canopy
    for (let j = 0; j < 3; j++) {
      const foliageGeom = new THREE.SphereGeometry((1.5 + Math.random() * 0.5) * scale, 6, 6);
      const foliageMat = new THREE.MeshStandardMaterial({
        color: foliageColors[(i + j) % foliageColors.length],
        roughness: 0.85
      });
      const foliage = new THREE.Mesh(foliageGeom, foliageMat);
      const angle = (j / 3) * Math.PI * 2 + Math.random();
      foliage.position.x = Math.cos(angle) * 1.5 * scale;
      foliage.position.z = Math.sin(angle) * 1.5 * scale;
      foliage.position.y = canopyY + Math.random() * 2 * scale;
  ...
  console.log("Optimized realistic trees created");
}

// Create realistic cliff formations
function createMassiveCliffs() {
  console.log("Creating cliff formations...");
  const waterLevel = window.suggestedWaterLevel || config.waterPlane?.height || 45;

  // Create 8-10 cliff formations
  for (let i = 0; i < 8; i++) {
    ...
    if (height > waterLevel + 10) {
      // Create realistic cliff wall - not too massive
      const cliffWidth = 8 + Math.random() * 12; // Realistic width
      const cliffHeight = 12 + Math.random() * 18; // Realistic height (10-30 units)
      const cliffDepth = 5 + Math.random() * 8;

      const cliffGeom = new THREE.BoxGeometry(cliffWidth, cliffHeight, cliffDepth);
      // Add rough rocky texture appearance
      const cliffMat = new THREE.MeshStandardMaterial({
        color: 0x8a8a90, // Lighter grey for visibility
        roughness: 1.0, // Very rough for rock
        metalness: 0.0,
        flatShading: true // Angular rocky look
      });
      ...
      // Add some smaller cliff pieces around it for natural look
      for (let j = 0; j < 2; j++) {
        const smallWidth = 5 + Math.random() * 8;
        const smallHeight = 8 + Math.random() * 12;
        const smallDepth = 4 + Math.random() * 6;
        const smallGeom = new THREE.BoxGeometry(smallWidth, smallHeight, smallDepth);
        const small = new THREE.Mesh(smallGeom, cliffMat);
        small.position.set(
  ...
  console.log("Massive cliffs created");
}
...
const terrainMaterial = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    void main() {
      ...
      // Path generation - follow valleys and flat areas
      float pathVal = pathNoise(vWorldPosition.xz);

      // Bias paths to appear in lower, flatter areas (realistic road placement)
      float heightBias = smoothstep(30.0, 80.0, height); // Prefer lower elevations
      float slopeBias = smoothstep(0.1, 0.4, slope); // Avoid steep slopes
      float terrainBias = 1.0 - (heightBias * 0.6 + slopeBias * 0.4);

      float pathMask = 1.0 - smoothstep(pathWidthMin, pathWidthMax, pathVal);
      pathMask *= terrainBias; // Paths only in suitable terrain

      // Rock on steep slopes and high elevations
      vec3 rock = dirtBase * rockColor;

      // Slope-based rock (more aggressive on cliffs)
      float slopeMask = smoothstep(rockSlopeThreshold - 0.2, rockSlopeThreshold, slope);
      slopeMask *= 0.6 + noise(vWorldPosition.xz * 0.03) * 0.6; // More variation

      // Height-based rock (high elevations)
      float heightRockMask = smoothstep(rockHeightMin - 20.0, rockHeightMin + 10.0, height);
      heightRockMask *= 0.7 + noise(vWorldPosition.xz * 0.02) * 0.4;

      // Combine slope and height rock masks
      float rockMask = max(slopeMask, heightRockMask * 0.8);
      ...
      // Rock on steep slopes and high elevations - apply after paths
      baseColor = mix(baseColor, rock, rockMask);
...
// OLD tree materials - now using ez-tree which generates its own materials
...
// Materials still used by bushes and ground cover
const bushMaterials = config.bushes.colors.map(
  (c) => new THREE.MeshStandardMaterial({ color: c, roughness: 0.85 }),
);

// OLD PROCEDURAL TREE GEOMETRIES - Now using ez-tree instead
// These are kept for bushes/ground cover but trees use ez-tree

// Create deformed sphere geometry once - used for bushes
...
function createInstancedDeciduousTrees() {
  ...
  console.log(`Creating ${positions.length} deciduous trees with ez-tree...`);
  console.log("Tree class:", Tree);
  ...
  positions.forEach((pos, i) => {
    ...
    try {
      // Create ez-tree instance
      const tree = new Tree();
      console.log(`Tree ${i} created successfully`);

      // Configure tree parameters from config
      tree.options.seed = dtc.ezTree.baseSeed + i;
      tree.options.trunk.length = dtc.ezTree.trunk.length * scale;
      tree.options.trunk.radius = dtc.ezTree.trunk.radius * scale;
      tree.options.branch.levels = dtc.ezTree.branch.levels;
      tree.options.branch.children = dtc.ezTree.branch.children;
      tree.options.branch.angle = dtc.ezTree.branch.angle;
      tree.options.branch.length = dtc.ezTree.branch.length;
      tree.options.branch.taper = dtc.ezTree.branch.taper;
      tree.options.branch.gnarliness = dtc.ezTree.branch.gnarliness;
      tree.options.leaves.type = dtc.ezTree.leaves.type;
      tree.options.leaves.count = dtc.ezTree.leaves.count;
      tree.options.leaves.size = dtc.ezTree.leaves.size * scale;
      tree.options.bark.type = dtc.ezTree.bark.type;
      tree.options.bark.tint = dtc.ezTree.bark.tint;
      tree.options.leaves.tint = dtc.ezTree.leaves.tint;
      ...
      scene.add(tree);
      console.log(`Deciduous tree ${i} added to scene at`, pos);
    } catch (error) {
      console.error(`Error creating deciduous tree ${i}:`, error);
    }
  });
  console.log("Deciduous tree creation complete");
}
...
function createInstancedCypressTrees() {
  ...
  console.log(`Creating ${positions.length} conifer trees with ez-tree...`);
  ...
  positions.forEach((pos, i) => {
    ...
    try {
      // Create ez-tree instance for conifer/pine
      const tree = new Tree();

      // Configure tree parameters for conifer appearance
      tree.options.seed = ctc.ezTree.baseSeed + i;
      tree.options.trunk.length = ctc.ezTree.trunk.length * scale;
      tree.options.trunk.radius = ctc.ezTree.trunk.radius * scale;
      tree.options.branch.levels = ctc.ezTree.branch.levels;
      tree.options.branch.children = ctc.ezTree.branch.children;
      tree.options.branch.angle = ctc.ezTree.branch.angle;
      tree.options.branch.length = ctc.ezTree.branch.length;
      tree.options.branch.taper = ctc.ezTree.branch.taper;
      tree.options.branch.gnarliness = ctc.ezTree.branch.gnarliness;
      tree.options.leaves.type = ctc.ezTree.leaves.type;
      tree.options.leaves.count = ctc.ezTree.leaves.count;
      tree.options.leaves.size = ctc.ezTree.leaves.size * scale;
      tree.options.bark.type = ctc.ezTree.bark.type;
      tree.options.bark.tint = ctc.ezTree.bark.tint;
      tree.options.leaves.tint = ctc.ezTree.leaves.tint;
      ...
      scene.add(tree);
      console.log(`Conifer tree ${i} added to scene at`, pos);
    } catch (error) {
      console.error(`Error creating conifer tree ${i}:`, error);
    }
  });
  console.log("Conifer tree creation complete");
}
